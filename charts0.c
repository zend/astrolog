...
void DisplayCredits()
{
  ...
  PrintW(sz, kLtGray);
  sprintf(sz, "(%s)", szAddressCore); PrintW(sz, kCyan);
  PrintW(NULL, 0);
...
void PrintObjects()
{
  ...
  for (i = 1; i <= oNorm; i++) if (!ignore[i]) {
    ...
    if (j) {
      sprintf(sz, "%c%c%c", chSig3(j)); PrintSz(sz);
    } else
      PrintSz("   ");
    PrintSz("  ");
    if (k) {
      sprintf(sz, "%c%c%c", chSig3(k)); PrintSz(sz);
    } else
      PrintSz("   ");
    PrintTab(' ', 5);
    if (j) {
      sprintf(sz, "%c%c%c", chSig3(Mod12(j+6))); PrintSz(sz);
    } else
      PrintSz("   ");
    PrintSz("  ");
    if (k) {
      sprintf(sz, "%c%c%c", chSig3(Mod12(k+6))); PrintSz(sz);
    } else
      PrintSz("   ");
    PrintTab(' ', 5);
    j = exalt[i];
    ...
    if (j) {
      ...
      PrintSz("   ");
    if (FCusp(i)) {
      sprintf(sz, "  House Cusp #%d", i-cuspLo+1); PrintSz(sz);
    } else if (FUranian(i)) {
...
void PrintAspects()
{
  ...
  for (i = 1; i <= cAspect; i++) {
    ...
    sprintf(sz, "%2d %-15s(%s) %6.2f +/- %1.0f degrees - %s\n",
      ...
      rAspAngle[i], rAspOrb[i], szAspectGlyph[i]); PrintSz(sz);
  }
  AnsiColor(kDefault);
}
